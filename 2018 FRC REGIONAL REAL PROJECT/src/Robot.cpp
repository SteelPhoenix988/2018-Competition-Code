...
public:
	Robot() {
		...
		double limit_input (double, double);
...
//		}
	...
	void OperatorControl() override {
		...
		while (IsOperatorControl() && IsEnabled()) {
				...
				if(fabs(dead_band(controller->GetRawAxis(0)))>0.5 || fabs(dead_band(controller -> GetRawAxis(1)))>0.5 || fabs(dead_band(controller->GetRawAxis(4))) > 0.5 )
				{
					time_counter += 0.005;
				}
				//if any (hence the or) input is recieved, then the counter keeps incrementing (correspnds with motor update)
				// absolute value (fabs for floating points) is taken to ensure that the time_counter increments even when input is negative (when the driver turns the motors the other direction).
				...
				//every half-second the drive_limiter is increased by 10%, reaching drivers' wanted speed eventually

				if(drive_limiter>1)
				{
					drive_limiter = 1.0;
				}
				// don't want the limiter to be over 100%

				if(fabs(dead_band(controller->GetRawAxis(0)))<0.3 && fabs(dead_band(controller->GetRawAxis(1)))< 0.3 && fabs(dead_band(controller->GetRawAxis(4)))<0.3)
				{
					time_counter = 0;
					drive_limiter = 0.75;
				}
				...
				(
						...
						.5*limit_input(dead_band(controller->GetRawAxis(5)),drive_limiter)*lftbump_limiter
				);
...
private:
		...
		double limit_input(double joystick, double drive_limiter)
		{
			if (fabs(joystick)>drive_limiter)
			{
				return drive_limiter * joystick / fabs(joystick);
			}
